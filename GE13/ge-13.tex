...
\begin{example}
	...
	Examine its numerical flux function: conservative? consistent?
	Verify that the \emph{Lax-Friedrichs} numerical flux function is consistent,
	including Lipschitz continuous.
\end{example}
...
\begin{example}
	...
	\begin{center}
		\begin{enumerate}
			\item[\text{Step-1:}] First, update the half-advanced-in-time point $u^{n+1/2}_{j+1/2}$ by using
			      \begin{align}
				      u^{n+1/2}_{j+1/2}
				      = \frac{1}{2} \left(u_{j+1}^n + u_j^n \right) -
				      \frac{\Delta t}{2\Delta x}
				      \left( f\left(u_{j+1}^n\right) -f\left(u_j^n\right) \right),
			      \end{align}
			\item[\text{Step-2:}] Then, update the fully-advanced-in-time point $u^{n+1}_{j}$ by using
			      \begin{align}
				      u_j^{n+1}         
				      = u_j^n -
				      \frac{\Delta t}{\Delta x}
	...
	it can be written in the \emph{conservation form}. 
...
\begin{enumerate}
	\item Conservation form: A numerical scheme applied for the conservation law
	      ...
	      \begin{align}
		      \begin{split}
			      ...
			      \frac{\Delta t}{\Delta x}
			      \left(  
			      \frac{1}{2} \left\lbrace f(u_j^n) + f(u_{j+1}^n) \right\rbrace  
			      - 
			      ...
			      + 
			      \frac{\Delta t}{\Delta x}
			      \left (  \frac{1}{2} \left\lbrace f(u_{j-1}^n) + f(u_{j}^n) \right\rbrace 
			      - \frac{\Delta t}{2\Delta x} a^2 (u_{j-1}^n, u_{j}^n) (u_{j}^n - u_{j-1}^n ) \right).
		      \end{split}
	      \end{align}
	      Then, this form can be written as follows
	      \begin{align*}
		      u_j^{n+1} 
		      = u_j^n 
		      - 
	      ...
	      \begin{align*}
		      \begin{split}
			      ...
			      =  \frac{1}{2}
			      \left\lbrace f(u_j^n) + f(u_{j+1}^n) \right\rbrace  
			      - \frac{\Delta t}{2\Delta x}
			      a^2 (u_j^n, u_{j+1}^n) (u_{j+1}^n - u_j^n ), \\
			      \tilde{f}_{j-1/2}^n 
			      & = \tilde{f}(u_{j-1}^n, u_{j}^n) = 
			      \frac{1}{2} \left\lbrace f(u_{j-1}^n) + f(u_{j}^n) \right\rbrace 
			      - \frac{\Delta t}{2\Delta x}
			      a^2 (u_{j-1}^n, u_{j}^n) (u_{j}^n - u_{j-1}^n ).
		      \end{split}
	      \end{align*}
	      Finally, the conservative numerical flux function is then given by 
	      \begin{align*}
		      f(u, v)  =  \frac{1}{2}
		      \left\lbrace f(u) + f(v) \right\rbrace  
		      - \frac{\Delta t}{2 \Delta x}
		      a^2 (u,v)(u-v).
	      \end{align*}
	\item Consistent numerical flux function:\\
	      ...
	      \begin{align*}
		      F(u,v)=F(u,u) = f(u).
	      \end{align*}
	      This clearly shows that the numerical flux function is consistent 
	      with the continuous flux function.  
\end{enumerate}
...
\begin{example}
	...
	the schemes can be written in the \emph{conservation form}. 
	Examine also if its numerical flux function is consistent.
\end{example}
...
Approach:
\begin{enumerate}
	\item The two-step \emph{Lax-Wendroff} scheme for the conservation law
	      ...
	      \begin{align*}
		      ...
		      u_j^{n+1}         & = 
		      u_j^n -\lambda \left( f\left(u_{j+1/2}^{n+1/2}\right)
	      ...
	      \begin{align*}
		      \begin{split}
			      ...
			      - \frac{\lambda}{2} \left( f(u_{j}^n) - f(u_{j-1}^n) \right) 
			      \right).
		      \end{split}
	      \end{align*}
	...
	\item For the \emph{MacCormack} scheme, the conservative flux function is given by
	      \begin{align*}
		      F(u,v) = \frac{1}{2}
		      \left( 
		      f(v) + f\left ( u - \lambda \left( f(v) - f(u) \right ) \right )
		      \right).
	      \end{align*}
	      Finally, it can be trivially shown that the above flux function is consistent.
\end{enumerate}
