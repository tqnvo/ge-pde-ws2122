...
NumPDE $\cdot$ GE15-extra $\cdot$ WS21/22 
\hspace{280pt} 
...
\begin{document}
\begin{center}
	\section*{Global Exercise - 15-extra}
\end{center}
\begin{center}
	Tuan Vo
\end{center}
\begin{center}
	$04^{\text{th}}$ February 2022
\end{center}
...
%------------------------------------------------------------------------------
\section{How the convexity property of $f(u)$ guarantees both
  $C_{j+1/2} \geq 0$ and $D_{j-1/2} \geq 0$ at the same time}
 (\textcolor[rgb]{0.8,0.0,0.0}{Continued from Example 10 $\cdot$ GE15 $\cdot$ $02^{\text{nd}}$ February 2022})\\

In order to show the scheme has TVD property,
...
can be shown by using convexity of flux function $f$. We now examine $C_{j+1/2}$.
The analysis is as follows

\begin{enumerate}
	\item Case 1: According to \rfig{convexityfu_case11}
	      and \rfig{convexityfu_case12},
	      if $\Big(f^\prime(u_L) \geq 0 \wedge  f^\prime(u_R) \geq 0\Big)
		      \vee \Big(f^\prime(u_L) \geq 0 \geq f^\prime(u_R) \wedge [f]/[u]>0\Big)$
	      then
	      \begin{align*}
		      C_{j+1/2} = 0 \quad \checkmark
	      \end{align*}
	      which satisfies $C_{j+1/2} \geq 0$.
	      \inputfig{floats/convexityfu_case11}{convexityfu_case11}
	      \inputfig{floats/convexityfu_case12}{convexityfu_case12}
	      
	\item Case 2.1: According to \rfig{convexityfu_case21},
	      if $f^\prime(u_L) \leq 0 \wedge f^\prime(u_R) \leq 0\ (\star)$ then
	      \begin{align*}
		      C_{j+1/2} = - \lambda \dfrac{f(u_R) - f(u_L)}{u_R-u_L}
		      = -\lambda \alpha.
	      \end{align*}
	      \inputfig{floats/convexityfu_case21}{convexityfu_case21}
	      \begin{itemize}
		      \item $f(u)$ convex with $(\star)$ leads to $f(u)$ decreasing at
		            both $u_{L}$ and $u_{R}$
		            \begin{itemize}
			            \item[$\rightarrow$] If $u_{L} > u_{R}$ then $\alpha<0$.
			                  Hence, $C_{j+1/2} > 0 \quad \checkmark$
			            \item[$\rightarrow$] If $u_{L} < u_{R}$ then $\alpha<0$.
			                  Hence, $C_{j+1/2} > 0 \quad \checkmark$
		            \end{itemize}
		      \item $f(u)$ concave with $(\star)$ leads to $f(u)$ decreasing at
		            both $u_{L}$ and $u_{R}$
		            \begin{itemize}
			            \item[$\rightarrow$] If $u_{L} > u_{R}$ then $\alpha<0$.
			                  Hence, $C_{j+1/2} > 0 \quad \checkmark$
	...
	\item Case 2.2: According to \rfig{convexityfu_case22}, if $\Big(f^\prime(u_L) \geq 0 \geq f^\prime(u_R)\Big) \wedge \Big( [f]/[u]<0 \Big)
		      \ (\star\star)$ then
	      \begin{align*}
		      C_{j+1/2} = - \lambda \dfrac{f(u_R) - f(u_L)}{u_R-u_L}
		      = -\lambda \alpha.
	      \end{align*}
	      \inputfig{floats/convexityfu_case22}{convexityfu_case22}
	      \begin{itemize}
		      \item $f(u)$ convex with the first condition in $(\star\star)$ leads to $f(u)$
		            decreasing at $u_{R}$ and increasing at $u_{L}$.
		            Hence, it must be $u_{L}>u_{R}$, which implies a shock.
		            Besides, the second condition $[f]/[u]<0$ implies also
		            an existence of shock, which is moving to the left
		            in this case.
		            \begin{enumerate}
			            \item[$\rightarrow$]
			                  If $u_{L} > u_{R}$ and $f(u_{L}) > f(u_{R})$
			                  then $\alpha>0$.
			                  Hence, $C_{j+1/2} < 0 \quad \lightning$
			            \item[$\rightarrow$]
			                  If $u_{L} > u_{R}$
			                  and $f(u_{L}) < f(u_{R})$
			                  then $\alpha<0$.
			                  Hence, $C_{j+1/2} > 0 \quad \checkmark$
		            \end{enumerate}
		            Therefore, the convexity of $f(u)$ gives us the possibility of
		            $C_{j+1/2} > 0$.
		      \item $f(u)$ concave with the first condition in $(\star\star)$ leads to $f(u)$
		            decreasing at $u_{R}$ and increasing at $u_{L}$.
		            Hence, it must be $u_{L}<u_{R}$, which does not imply a shock,
		            but rather a rarefaction solution.
		            Besides, the second condition $[f]/[u]<0$ implies
		            an existence of shock, which is moving to the left
		            in this case. This is a \textbf{contradiction} for the case of
		            $f(u)$ concave.
		            Therefore, the concavity of $f(u)$ does not lead us
		            to the possibility of $C_{j+1/2} > 0$.
	      \end{itemize}
	      
	\item  Case 3: According to \rfig{convexityfu_case3}, if $f^\prime(u_L) < 0 <  f^\prime(u_R) \ (\star\star\star)$
	      then
	      \begin{align*}
		      C_{j+1/2} = - \lambda \dfrac{f(u_s) - f(u_L)}{u_R-u_L}
		      = -\lambda \alpha.
	      \end{align*}
	      \inputfig{floats/convexityfu_case3}{convexityfu_case3}
	      \begin{itemize}
		      \item $f(u)$ convex with the condition $(\star\star\star)$
		            leads to $f(u)$
		            decreasing at $u_{L}$ and increasing at $u_{R}$.
		            Hence, it must be $u_{R}>u_{L}$, which also implies a rarefaction.
		            Therefore, we obtain $u_{s}\in[u_{L},u_{R}]$
		            which leads to $f(u_{s})<f(u_{R,L})$.
		            As a consequence, $\alpha<0$.
		            Finally, the convexity of $f(u)$ do lead us
		            to the possibility of $C_{j+1/2} > 0$.
		      \item $f(u)$ concave with the condition $(\star\star\star)$
		            leads to $f(u)$
		            decreasing at $u_{L}$ and increasing at $u_{R}$.
		            Hence, it must be $u_{R}<u_{L}$, which also implies a shock.
		            Therefore, we obtain $u_{s}\in[u_{R},u_{l}]$
		            which leads to $f(u_{s})>f(u_{R,L})$.
		            As a consequence, $\alpha<0$.
		            Finally, the convexity of $f(u)$ do lead us also
		            to the possibility of $C_{j+1/2} > 0$.
	      \end{itemize}
\end{enumerate}

Therefore, by combining case $1$, case $2.1$, case $2.2$, and case $3$ altogether,
we recognize that only the convexity of $f$ does lead us to the possibility
of $C_{j+1/2} > 0$, while the concavity of $f$ has failed at case $2.2$

In case of $D_{j-1/2}$ the argument will be proceeded the same as for
case $C_{j+1/2}$, meaning that
the convexity of $f$ does lead us to the possibility
of $D_{j-1/2} > 0$, while the concavity is not so.
...
% % \clearpage
% % %------------------------------------------------------------------------------
% % \section{A small note about Roe's matrix}
% % \begin{example}
% % 	Examine Roe's matrix of isothermal system of equations.
% % \end{example}
% % Roe's solver:\\
% % P.L.Roe [1981], 
...
% \end{example}
% \inputfig{floats/structureFVM}{structureFVM}
...
% 	\end{equation*}
% \end{example}
% \inputfig{floats/limiter1}{limiter1}
...
% 	Examine $u_{L} > u_{R}$.
% \end{example}
% \inputfig{floats/RiemannAllCasesLR_set1}{RiemannAllCasesLR_set1}
% Schock solution
...
% 	Examine $u_{L} > u_{R}$.
% \end{example}
% \inputfig{floats/RiemannAllCasesLR_set2}{RiemannAllCasesLR_set2}
% Schock solution
...
% 	\end{enumerate}
% 	% \hint{A numerical flux function $g(u,v)$ is monotone if it is monotonically increasing and decreasing
